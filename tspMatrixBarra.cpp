...
string repetirChar(string c, int num)
{
    string S = "";
    while(num--)
        S += c;
...
{
    int iteration = (int)percent;
    int filledLength = (int)(50*iteration / 100.0);
    string bar = repetirChar("█", filledLength) + repetirChar("-", 50 - filledLength);
    printf("\rProgresso [%s] %.1f%% Completo\r", bar.c_str(), percent);
}
